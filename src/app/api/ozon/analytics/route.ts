...
import { getOzonHeaders, ozonFetch, OzonRateLimitError } from '@/lib/ozon/gate';
import { fetchPostingsForRange } from '@/lib/ozon/postings';
...
async function fetchRealOrders(dateFrom: string, dateTo: string, headers: Record<string, string>, store: string = 'store1') {
  try {
    ...
    const postings = await fetchPostingsForRange(sinceISO, toISO, headers, { maxPostings: 2500 });

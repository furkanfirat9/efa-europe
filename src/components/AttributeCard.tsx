      ...
      <div className="p-4 space-y-2.5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {attribute.is_aspect && (
              <span className="px-2 py-0.5 rounded-sm text-[10px] font-medium bg-purple-100 text-purple-700">
                Varyant
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 pt-1 text-[10px] font-medium">
          <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded-sm font-mono">
            ID: {attribute.id}
          </span>
          <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded-sm font-mono">
            Tip: {attribute.type}
          ...
          {isDictionary ? (
            <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 px-2 py-0.5 rounded-sm font-medium border border-blue-100">
              <BookOpen className="w-3 h-3 text-blue-600" />
              <span>Sözlük ID: {attribute.dictionary_id}</span>
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-sm font-medium border border-emerald-100">
          ...
          {attribute.is_collection ? (
            <span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-sm border border-indigo-100">
            ...
            <div className="p-4 pt-2 space-y-3 bg-white border-t border-slate-100">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 placeholder:text-slate-400 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                />
                {isSearching && (
              ...
              {/* Error state */}
              {error && (
                <div className="text-[11px] text-red-600 bg-red-50 p-2 rounded-sm border border-red-100">
                  {error}
                </div>
              )}
                  ...
                  {searchResults.length === 0 && !isSearching ? (
                    ...
                    <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto p-1">
                      {searchResults.map((item) => (
                        <div
                          key={item.id}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50/80 border border-blue-200/80 text-xs text-blue-900 hover:bg-blue-100 transition-colors"
                        >
                          {item.picture && (
                            <img
                              src={item.picture}
                          ...
                          <span className="text-[10px] font-mono text-blue-500 bg-white/80 px-1 rounded-sm">
                            {item.id}
                          </span>
                        </div>
                      ))}
                  ...
                  {loadingValues && values.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto p-1">
                      {values.map((item) => (
                        <div
                          key={item.id}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 border border-slate-200 text-xs text-slate-800 hover:bg-slate-200/70 transition-colors"
                        >
                          {item.picture && (
                            <img
                              src={item.picture}
                          ...
                          <span className="font-medium">{item.value}</span>
                          <span className="text-[10px] font-mono text-slate-500 bg-white px-1 rounded-sm border border-slate-200">
                            {item.id}
                          </span>
                        </div>
                      ))}

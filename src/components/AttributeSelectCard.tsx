  ...
  return (
    <div
      className={`p-4 rounded-xl border space-y-2.5 transition-all relative ${
        isDescription
          ? 'md:col-span-2 bg-linear-to-br from-slate-50 to-blue-50/20 border-blue-200'
          : isLocked
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-1.5">
            ...
            {isDescription && (
              <span className="px-2 py-0.5 rounded-sm text-[10px] font-bold bg-blue-100 text-blue-800 border border-blue-200 flex items-center gap-1">
                <FileText className="w-3 h-3 text-blue-600" /> Ürün Açıklaması (Аннотация - SEO)
              </span>
            )}
            {isSeriesModelCard && (
              <span className="px-2 py-0.5 rounded-sm text-[10px] font-bold bg-purple-100 text-purple-800 border border-purple-200 flex items-center gap-1">
                🔗 Seri Kart Kodu (Varyant Birleştirme)
              </span>
            )}
            {isNamingTemplate && (
              <span className="px-2 py-0.5 rounded-sm text-[10px] font-bold bg-amber-100 text-amber-900 border border-amber-200 flex items-center gap-1">
                🏷️ Vitrin Model Adı (Şablon)
              </span>
            )}
            {isIgnored && (
              <span className="px-1.5 py-0.2 rounded-sm text-[10px] font-bold bg-slate-200 text-slate-500 border border-slate-300 flex items-center gap-0.5">
                <Lock className="w-2.5 h-2.5 text-slate-400" /> Pasif (Kullanılmıyor)
              </span>
            )}
            {isWarranty && (
              <span className="px-1.5 py-0.2 rounded-sm text-[10px] font-bold bg-slate-200 text-slate-800 border border-slate-300 flex items-center gap-0.5">
            ...
            {isMultiSelect && !isLocked && (
              <span className="px-1.5 py-0.2 rounded-sm text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 flex items-center gap-0.5">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {isLocked ? (
            <span className="px-2 py-0.5 rounded-sm text-[10px] font-bold bg-slate-700 text-white flex items-center gap-1">
              <Lock className="w-2.5 h-2.5" /> Salt Okunur
            </span>
          ) : attribute.isRequired ? (
            <span className="px-2 py-0.5 rounded-sm text-[10px] font-bold bg-amber-500 text-white">
              Zorunlu
            </span>
          ) : (
            <span className="px-2 py-0.5 rounded-sm text-[10px] font-medium bg-slate-100 text-slate-500">
              Opsiyonel
            </span>
          )}

          {isFilled && (
            <span className="px-2 py-0.5 rounded-sm text-[10px] font-bold bg-emerald-100 text-emerald-800 flex items-center gap-1">
      ...
      {isLocked ? (
        <div className="w-full p-2.5 rounded-lg bg-slate-200/80 border border-slate-300 text-xs font-bold text-slate-800 flex items-center justify-between select-none">
          <div className="flex items-center gap-2">
            ...
            {isWarranty && (
              <span className="text-[10px] font-mono text-slate-600 bg-white/90 px-1.5 py-0.2 rounded-sm border border-slate-300">
                ID: 972120566
              </span>
            )}
          </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 min-w-0 pr-2">
              {currentSelectedValues.length > 0 ? (
                currentSelectedValues.map((val, idx) => (
                  <span
                    key={`${val.value}-${idx}`}
                    className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-100/80 text-blue-900 border border-blue-200 rounded-md text-xs font-medium"
                  >
                    <CheckCircle2 className="w-3 h-3 text-emerald-600 shrink-0" />
                    <span className="truncate max-w-[140px]">{val.value}</span>
                    {val.dictionaryValueId && (
                      <span className="text-[9px] font-mono text-blue-600 bg-white/80 px-1 rounded-sm">
                        {val.dictionaryValueId}
                      </span>
                    )}
                    {isMultiSelect && (
                      <button
                        ...
                        className="hover:text-red-600 hover:bg-blue-200/60 p-0.5 rounded-sm transition-colors"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    )}
            ...
            <div className="flex items-center gap-1 text-slate-400 shrink-0">
              {isFilled && (
                <button
                  ...
                  className="p-1 hover:text-red-500 hover:bg-red-50 rounded-sm"
            ...
            <div className="absolute left-0 right-0 top-full mt-1.5 z-50 bg-white border border-slate-200 rounded-xl shadow-xl p-2.5 space-y-2 max-h-72 flex flex-col animate-in fade-in-50 zoom-in-95">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-7 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                />
                {isSearching && (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 truncate">
                            ...
                            {item.picture && (
                              <img src={item.picture} alt="" className="w-4 h-4 rounded-sm object-cover" />
                            )}
                            <span className="truncate">{item.value}</span>
                          </div>

                          <span className="text-[10px] font-mono text-slate-400 shrink-0 bg-slate-50 px-1 py-0.2 rounded-sm">
                            {item.id}
                          </span>
                        </div>
                      );
        ...
        <div className="space-y-1.5">
          <textarea
            ...
            className="w-full px-3.5 py-2.5 rounded-xl text-xs leading-relaxed font-normal focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 bg-white border border-slate-300 text-slate-900 resize-y"
          />
        ...
        <div className="space-y-1">
          <input
            ...
            className={`w-full px-3 py-2 rounded-lg text-xs font-medium focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 ${
              isLocked

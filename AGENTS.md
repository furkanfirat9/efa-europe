...
- **Toplu Yükleme (`/toplu-yukle`):**
  ...
  - **Açık kategoriler:** Yapay zekâ yalnızca `ALLOWED_ROOT_DEPARTMENTS` bölümlerini ve `ALLOWED_EXTRA_CATEGORIES` / `ALLOWED_EXTRA_TYPES` dallarını görür (`src/app/api/ai/bulk-detect-categories/route.ts`). Listede olmayan bir ürün türü satılacaksa önce dalı buraya eklenir; yoksa yapay zekâ en yakın açık kategoriyi seçer (araba ampulleri "Украшение на машину" olmuştu). Bütün bölümü açma: istem büyür.
...
- **Ozon Tam Sayı Fiyat Kuralı (Integer Price):**
  - Ozon API'si kuruşlu/ondalıklı değer kabul etmediğinden ve Amazon alış fiyatı Euro (€) olup Ozon fiyatı USD ($) olarak yüklendiğinden; Amazon fiyatı günlük ECB EUR/USD referans kuruyla dolara çevrilir, çarpanla çarpılıp tam sayıya yuvarlanır: `Math.round(buyPriceEur * ecbRate * OZON_PRICE_MULTIPLIER)` (`src/lib/fx/ecb.ts`, 3 saat önbellek). Çarpan `src/lib/amazon/types.ts` içindeki `OZON_PRICE_MULTIPLIER`'dır (şu an **4**, 2026-09-23'te 3'ten yükseltildi); ekrandaki "×4" yazıları da buradan gelir. Eskiden kur yerine sabit `1.15` vardı; kur ondan uzaklaştıkça çarpan kayıyordu. ECB'ye ulaşılamazsa `1.15` ile hesaplanır ve tarama sonucu uyarı taşır; kullanılan kur ekranda gösterilir.
...
- **Bileşenler:** `src/components/shadcn/` altındadır (`components.json` → `ui: @/components/shadcn`). Yeni UI önce buradaki bileşenlerle yazılır. Eksik bileşen CLI ile eklenir: `npx shadcn@latest add <bileşen>`. Kendi görsel stilini icat etme, shadcn bileşenlerinin varsayılan class'larını keyfi değiştirme.
  ...
  - Var olan bir dosyaya bağımlı bileşen eklenirken CLI üzerine yazma sorusu sorup takılır; `--overwrite --yes` ile çalıştırılır (sonra `cn` düzeltmesi tekrarlanır).
...
- **Tema:** shadcn değişkenleri `:root`'ta global tanımlıdır; portal edilen içerik (select, dialog, popover) de temayı alır. Font Geist / Geist Mono (`layout.tsx`).
  ...
  - Body'ye portal edilen yüzeyler (sheet, dialog, popover, dropdown, select, command) metin rengini `globals.css`'teki bir taban kuralından alır; yeni bir portal bileşeni eklenirse `data-slot`'u o listeye eklenir.
  - `dark:` varyantı `.dark` sınıfına bağlıdır (`@custom-variant dark`); işletim sistemi koyu modda olsa da shadcn'in koyu stilleri devreye girmez. Bu satır kaldırılmamalı.
...
4. [x] **TanStack Table:** Siparişler sayfası ele alınırken eklenir (sıralama, filtre, sayfalama); tablo görünümü shadcn `Table` bileşeniyle.
